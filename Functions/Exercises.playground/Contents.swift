...
let digits = CharacterSet.decimalDigits
 ...
 func idek(_ val: String) -> Bool {
    for character in val.unicodeScalars {
        if !digits.contains(UnicodeScalar(unicodeScalarLiteral: character)) {
            return false
        }
    }
    return true
...
let dirtyWordsArray = ["phooey", "darn", "drat", "blurgh", "jupiters", "argh", "fudge"]
 ...
 func removeDuttyWordies(_ dirty: [String]) -> [String] {
    var result: [String] = []
    for word in dirty {
        if word.count != 4 {
            result.append(word)
...
class MovieArchive {
    static func moviesByDirector( movies: [String:String], director: String) -> [String] {
 ...
 MovieArchive.moviesByDirector(movies: movies, director: "Christopher Nolan")
