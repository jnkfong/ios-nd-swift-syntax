...
let yourSentence = "The \(nounArray[0]) became a great \(nounArray[6])."
let yourSillySentence = "I should've bought a \(nounArray[index3]) instead of a \(nounArray[index4])"
//: ### Exercise 2
...
for (key, val) in dictionary {
    newestString = newestString.replacingOccurrences(of: key, with: val)
}
//: ### Exercise 8
...
//desired output = "$5.67"
let output = "$\(Float(numOfPennies) / 100)"
//: # Let or Var?
import UIKit
import Foundation
//: ### Exercise 9
...
let maximumTimeAllowed = 20 * 60
var timeRemaining = maximumTimeAllowed - currentTime
